...
impl LocalEngine {
    pub fn new() -> Result<Self> {
        ...
        let system_prompt = "You are a Linux System Architect assistant embedded in a Hyprland environment.
...
Always output commands in format [[CMD:action:args]] when executing system actions.".to_string();
        ...
        Ok(Self { model, tokenizer, device, logits_processor, system_prompt })
    }

    pub fn generate_stream(&mut self, user_input: &str, vector_store: Option<&VectorStore>, callback: impl Fn(String)) -> Result<()> {
        
        let mut context_injection = String::new();
        if let Some(store) = vector_store {
            if let Ok(results) = store.search(user_input, 2) {
                if !results.is_empty() {
        ...
        let full_system_prompt = format!("{}{}", self.system_prompt, context_injection);
